...
<template>
  <div class="rounded-lg border border-orange-500/40 bg-[var(--app-bg-card)] overflow-hidden">
    <div class="flex items-center gap-2 px-3 pt-2.5">
      <ShieldAlert :size="15" class="shrink-0 text-orange-500" />
      <span class="text-13px font-600 text-[var(--app-text-primary)]">需要你确认该操作</span>
      <LewTag :type="'light'" :color="riskColor(approval.riskLevel)" size="small" class="shrink-0">
        {{ riskText(approval.riskLevel) }}
      </LewTag>
      <span class="ml-auto min-w-0 truncate text-12px font-mono text-[var(--app-text-muted)]">
        {{ approval.toolName }}
      </span>
    </div>
    ...
    <div v-if="approval.preview" class="px-3 pt-2">
      <div class="rounded-md bg-[var(--app-bg-hover)] px-2.5 py-2">
        <div class="text-12.5px leading-relaxed text-[var(--app-text-primary)] break-all">
