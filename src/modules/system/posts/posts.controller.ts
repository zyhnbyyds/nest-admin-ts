...
export class PostsController {
  ...
  @Get(':id') @RequirePermissions('system:post:list') findOne(
    @Param('id', ParseIntPipe) id: number,
  ) {
    return this.posts.findOne(id);
  }
  @Post() @RequirePermissions('system:post:create') create(
    @Body() body: unknown,
    @Req() request: AuthRequest,
  ) {
    return this.posts.create(createSchema.parse(body), request.user.id);
  }
  @Patch(':id') @RequirePermissions('system:post:update') update(
    @Param('id', ParseIntPipe) id: number,
    @Body() body: unknown,
    @Req() request: AuthRequest,
  ) {
  ...
  @Delete(':id') @RequirePermissions('system:post:delete') remove(
    @Param('id', ParseIntPipe) id: number,
    @Req() request: AuthRequest,
  ) {
