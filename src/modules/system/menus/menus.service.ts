...
export class MenusService {
  ...
  async routes(userId: number): Promise<RouteNode[]> {
    const assignments = await this.database.db
      .select({ roleId: userRoles.roleId, isSystem: roles.isSystem })
      .from(userRoles)
    ...
    if (isSuperAdmin) {
      ...
      rows = await base
        .innerJoin(roleMenus, eq(menus.id, roleMenus.menuId))
        .where(and(inArray(roleMenus.roleId, roleIds), ...conditions))
        .orderBy(asc(menus.sort), asc(menus.id));
    }
  ...
  private async assertPermissionUnique(
    ...
    const conditions = [
      eq(menus.permission, permission),
      isNull(menus.deletedAt),
    ];
